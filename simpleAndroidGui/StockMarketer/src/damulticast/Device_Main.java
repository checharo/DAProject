			...
			try {
				...
				if (command.equals("peerlist")) {
					if (device.getPeers().isEmpty())
						...
						for (RemoteDevice peer : device.getPeers()) {
					...
					try {
						...
						System.err
								.println("usage: send <peerId> <header> <message>");
					...
					try {
						...
						if (device.getSharedResources().hasValue(key)) {
							System.err
									.println("The system already has a value for: "
											+ key);
						} else {
						...
						System.err
								.println("usage: new <resource_name>|<value>");
					} catch (NumberFormatException nfe) {
						System.err.println("value must be numerical");
					}

					/*
					 * resources, prints a list of the shared resources and
					 * their state IN THIS DEVICE
					 */

				} else if (command.startsWith("resources")) {
					HashMap<String, Integer> values = device
							.getSharedResources().getValues();
					if (values.isEmpty()) {
					...
					try {
						...
						System.err
								.println("usage: lock <resource_name>|<value>");
					} catch (NullPointerException npe) {
						/*
						 * Resource was wanted or held already, or does not
						 * exist
						 */
					...
					/*
					 * release, releases the lock for this resource and
